<template>
  <div class="infofacechild4" ref="infoFaceC4">
    <div class="info_face_left" ref="infoFaceLeft">
      ...
      <div
        v-show="!loading"
        @click.right.capture="infoC2RightMenuFun()"
        id="kline_container"
        ref="klineContainer"
      ></div>
    </div>
...
export default {
    ...
    updateSymbol(val, oldVal) {
      ...
      if (that.$store.state.otherCodeName == val[2]) {
        that.symbolInfo = {
          ...
          BV1:
            val[8] != "----"
              ? Number(val[8]).toFixed(that.$store.state.point)
              : val[8], // 买量
          SV1:
            val[9] != "----"
              ? Number(val[9]).toFixed(that.$store.state.point)
              : val[9], // 卖量
          ...
          change_rate:
            Number(((val[3] - val[14]) / val[14]) * 100).toFixed(2), // 幅度
          total_hand: "--", // 总手
          open: Number(val[11]).toFixed(that.$store.state.point), // 开盘
          turnover:
            val[15] != "----"
              ? Number(val[15]).toFixed(that.$store.state.point)
              : val[15], // 持仓
          high: Number(val[12]).toFixed(that.$store.state.point), // 最高
          p_clear:
            val[16] != "----"
              ? Number(val[16]).toFixed(that.$store.state.point)
              : val[16], // 昨结
  ...
  mounted() {
    ...
    that.infoC4GetKlineFun(that.$store.state.otherCodeName, that.time); // K线初始化

    that.getPointFun(that.$store.state.otherCodeName);
            ...
            for (let i = 0; i < res.data.length; i++) {
              if (res.data[i][5] != 0) {
                that.detail.unshift({
                  ...
                  price: res.data[i][2].toFixed(that.$store.state.point),
                  volume: res.data[i][5]
                });
              }
            }
